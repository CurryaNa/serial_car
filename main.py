...
if __name__ == '__main__':
    ...
    while True:
        flag = int(input("请输入您的选择：1.前往目标站点 2.读取文件中的指令 3.设置小车的速度 4.站点往复模式 5.按设定依次到达站点"))
        res = load("instruct.json")
        t1 = MyThread(ser)
        stop_Threads = False
        if flag == 1:
            ...
            num = res.get("4").get("目标站点")
            tim = res.get("4").get("loopTimes")
            t = MyThread(ser)
            t1.start()
            for i in range(tim):
